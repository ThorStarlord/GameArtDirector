...
**Do not use hex codes in `color_palette` arrays.** Ideogram processes named color descriptions more reliably than hex values. Hex codes have weak-to-no influence on generated output.
...
Rules:
- 5–12 entries for global `style_description` palette
- Up to 5 entries for per-element palettes
- Pair a memory color name with a material or light qualifier (e.g., "charcoal matte walls", "glacier LED strips")
...
| 5 | Hex codes in color_palette | Weak — largely ignored |
| 6 | Percentages (e.g. "30% floor") | Weak — imprecise approximation only |
...
4. **Color Palette** — 5–12 named color descriptions, no hex codes (see Color Palette Rules above)
...
10. **Element Palettes** — per-element named color palettes (up to 5 entries, no hex codes)
